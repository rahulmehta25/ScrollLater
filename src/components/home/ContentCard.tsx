...
export function ContentCard({ item, onClick }: { item: DemoItem; onClick?: () => void }) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className={cn(
        'group flex gap-3.5 p-3.5 bg-white border border-gray-100 rounded-xl hover:border-gray-200 transition-colors cursor-pointer',
        item.isRead && 'opacity-60'
      )}
    >
      <div
        className={cn(
          'flex-shrink-0 w-14 h-14 rounded-lg flex items-center justify-center transition-transform group-hover:scale-105',
        ...
        <div className="mt-1.5 flex items-center gap-1">
          {item.tags.slice(0, 3).map((tag) => (
            <motion.span
              ...
              className="px-1.5 py-0.5 bg-gray-50 text-gray-500 rounded text-[10px] cursor-default hover:bg-gray-100 hover:text-gray-700 transition-colors"
            >
              {tag}
            </motion.span>
          ))}
