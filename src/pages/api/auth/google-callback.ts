...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!session?.user) {
      ...
      // State is a CSRF token, not a JWT -- cannot recover session from it
      // Redirect to login
    ...
    const errCode = error instanceof Error && 'code' in error ? (error as Error & { code?: string }).code : undefined;
    ...
    } else if (errMessage.includes('fetch failed')) {
