...
async function generateAISuggestions(
  ...
  existingScheduled: Array<Record<string, unknown>>
): Promise<SchedulingSuggestion[]> {
  try {
    // Sanitize user content before inserting into prompts
...
Entries to schedule:
${sanitizedEntries.map((entry, index) => `${index + 1}. ${entry.content.substring(0, 100)}... (Category: ${entry.category}, Urgency: ${entry.urgency})`).join('\n')}
...
${existingScheduled.map(item => `- ${new Date(item.scheduled_for as string).toLocaleString()}`).join('\n')}
